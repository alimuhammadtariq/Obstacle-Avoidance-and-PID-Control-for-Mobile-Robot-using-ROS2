...
def PID():
    global regions_, eprevious, eI #Defined the golobal variables to global access.
    
    #calculate the value of error: e
    desired_val= 0.30
    current_val= regions_['right']
    
    e= desired_val-current_val
    print("Error: ", e)
  
    
    #calculate the integral Error EI (integeral)
    eI+=e
    print("Intgeral Error: ", eI)
    
    #calculate the differential error Ed (Derivative)
    ed= e-eprevious  
    eprevious=e


    #The optimal Values of Constants that I got on Turtle bot CSEEBurger_30
    
    Kp=0.4		
    KI=0
    kd=0.2

    # return the turn value for the robot to make the trajectory corrections
    PID = Kp*e + KI*eI + kd*ed

    # Return Value and print for debugging.
    print ("PID: ", PID)
    return PID	
